...
export default function ProfilePage() {
  ...
  const handleUpdateProfile = async (e) => {
    e.preventDefault();
    if (!isValidName) {
      return Swal.fire("Gagal", "Nama minimal 3 karakter dan hanya boleh berisi huruf.", "error");
    }
    setSaving(true);
    try {
      await api.put("/users/profile", { name: name.trim() });
      Swal.fire({ title: "Berhasil!", text: "Profil diperbarui.", icon: "success", timer: 1500, showConfirmButton: false });
  ...
  const handleUpdatePassword = async (e) => {
    ...
    if (!isPasswordSecure) {
      return Swal.fire("Error", "Password baru belum memenuhi kriteria keamanan!", "error");
    }
    setSaving(true);
    try {
        ...
        <Card className="p-8 lg:p-10">
          <div className="flex flex-col sm:flex-row items-center gap-8 mb-10 text-center sm:text-left">
            <div className="relative">
              <div className="w-24 h-24 rounded-full bg-[#011F7B] flex items-center justify-center text-3xl font-extrabold text-white shrink-0 shadow-lg shadow-[#011F7B]/20 border-4 border-white">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-2xl font-bold text-slate-900 mb-1 capitalize">{user?.name}</h2>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {isGoogleUser && (
                  <Badge variant="slate" className="flex items-center gap-1 normal-case">
          ...
          {editMode ? (
            <form onSubmit={handleUpdateProfile} className="space-y-6">
              ...
              <div className="flex gap-3">
                <Button type="submit" isLoading={saving} icon={HiOutlineSave}>Simpan</Button>
                <Button variant="danger" onClick={() => setEditMode(false)} icon={HiOutlineX}>Batal</Button>
              </div>
            </form>
          ) : (
            <Button variant="outline" className="w-full sm:w-auto" onClick={() => setEditMode(true)} icon={HiOutlinePencil}>
              Edit Profil
            </Button>
          )}
        </Card>
        ...
        <Card className="p-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isGoogleUser ? 'bg-blue-50 text-blue-500' : 'bg-red-50 text-red-500'}`}>
                {isGoogleUser ? <HiOutlineGlobeAlt size={24} /> : <HiOutlineLockClosed size={24} />}
              </div>
              <div>
                <h3 className="font-bold text-slate-900">Keamanan</h3>
                <p className="text-xs text-slate-500 font-medium">
                  {isGoogleUser ? "Akun Anda terhubung dengan Google." : "Perbarui kata sandi Anda secara berkala."}
                </p>
              </div>
            </div>
            
            {!isGoogleUser ? (
              <Button variant="outline" className="text-xs" onClick={() => setShowPwForm(!showPwForm)}>
              ...
              <div className="px-4 py-2 bg-slate-50 rounded-lg border border-slate-100 flex items-center gap-2">
                <FcGoogle size={16} />
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Google Managed</span>
              </div>
            )}
          </div>
          ...
          {isGoogleUser && (
            <div className="mt-8 pt-6 border-t border-slate-50">
              <p className="text-sm text-slate-500 font-medium leading-relaxed bg-blue-50/50 p-4 rounded-xl border border-blue-100/50">
                Karena Anda menggunakan akun Google, pengaturan keamanan dan kata sandi dikelola langsung melalui portal <a href="https://myaccount.google.com/security" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-bold">Keamanan Google</a>.
              </p>
            </div>
          )}

          {!isGoogleUser && showPwForm && (
            <form onSubmit={handleUpdatePassword} className="space-y-6 mt-8 pt-8 border-t border-slate-50">
              <Input label="Password Lama" type={showPass ? "text" : "password"} value={pwForm.currentPassword} onChange={(e) => setPwForm({ ...pwForm, currentPassword: e.target.value })} icon={<HiOutlineLockClosed />} required />
              ...
              <Input label="Konfirmasi Password Baru" type={showPass ? "text" : "password"} value={pwForm.confirmNewPassword} onChange={(e) => setPwForm({ ...pwForm, confirmNewPassword: e.target.value })} icon={<HiOutlineLockClosed />} required />
              
              <Button type="submit" variant="danger" isLoading={saving} className="w-full" icon={HiOutlineLockClosed}>
                Ubah Password
              </Button>
            </form>
          )}
