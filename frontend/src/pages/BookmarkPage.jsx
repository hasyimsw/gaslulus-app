    ...
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-primary mb-1">
          Bank Soal <span className="text-amber-500 font-bold">Saya</span>
        </h1>
      ...
      {loading ? (
        ...
        <Card className="text-center py-24 flex flex-col items-center">
          ...
          <p className="text-slate-500 font-medium max-w-sm mx-auto">
            Klik ikon bookmark saat mengerjakan ujian untuk menyimpan soal yang
            ingin dipelajari.
          </p>
        </Card>
      ) : (
                ...
                <Card className="overflow-hidden" hoverable={!isOpen}>
                  <div className="flex items-start gap-6 p-6">
                    ...
                    >
                      ...
                      <p className="font-semibold text-slate-800 leading-relaxed text-lg">
                        {q.question}
                      </p>
                    </div>
                    <div className="flex gap-2">
                        ...
                        <div className="p-6 lg:p-8 bg-slate-50/30">
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {q.options.map((opt, oi) => (
                              <div
                                key={opt.id}
                                className={`p-4 rounded-xl border-2 flex items-center gap-4 transition-all ${opt.isCorrect ? "bg-emerald-50 border-emerald-500 text-emerald-900" : "bg-white border-slate-50 text-slate-500"}`}
                              >
                                <span
                                  className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${opt.isCorrect ? "bg-emerald-500 text-white shadow-lg shadow-emerald-100" : "bg-slate-100 text-slate-400"}`}
                                >
                                  {String.fromCharCode(65 + oi)}
                                </span>
                                <span
                                  className={`font-bold ${opt.isCorrect ? "text-emerald-900" : "text-slate-600"}`}
                                >
                                  {opt.optionText}
                                </span>
                              </div>
